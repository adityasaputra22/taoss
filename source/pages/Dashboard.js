...
//  }
  ...
  updateRouter3 = () => {
    this.setState({     
      ...
      ymg: 84,
      id: 3,
      }, function() {this.setState({         lastvar: this.state.downlink.length - 2,
        lastvar2: this.state.uplink.length - 2,
        lastvar3: this.state.modem.length - 2}); if (this.state.uplinkChart === 'week') { this.getUplink('week') }
      if (this.state.downlinkChart === 'week') { this.getDownlink('week') }})
  }
...
testfunc() {
      ...
      if (this.props.downlink !== null) {
        if (this.props.downlink) {
          this.setState({
            ...
            isLoading1: false,
            lastvar: this.props.downlink.data.length - 1, 
          })
        }
      }
    }
      ...
      if (this.props.uplink !== null) {
        if (this.props.uplink) {
          this.setState({
            ...
            isLoading2: false,
            lastvar2: this.props.uplink.data.length - 1,
          })
        }
      }
    }
      ...
      if (this.props.modem !== null) {
        if (this.props.modem.status) {
          this.setState({
            ...
            isLoading3: false,
            lastvar3: this.props.modem.data.length - 1,
          })
        }
      }
    }
    ...
    if (this.state.downlinkChart === 'week') {
      ...
      return [
        ...
        {  x: Unix_timestamp(this.state.downlink.length ? this.state.downlink[this.state.ymg].timestamp ? this.state.downlink[this.state.ymg].timestamp : 0 : 0), y: parseInt(this.state.downlink.length ? this.state.downlink[this.state.ymg].sqf ? this.state.downlink[this.state.ymg].sqf : 0 : 0), label: this.state.downlink.length ? this.state.downlink[this.state.ymg].sqf ? this.state.downlink[this.state.ymg].sqf : 0 : 0}
          ...
          if (this.state.downlink.length > 13) {
            for(let i = this.state.downlink.length - 14;i<this.state.downlink.length;i++) {
            ...
            if (this.state.downlink.length < 14 && this.state.downlink.length > 0) {
  ...
  dataUplink = () => {
    if (this.state.uplinkChart === 'week') {
      return [
        {  x: this.state.uplink.length ? this.state.uplink[0].timestamp  : 0, y: this.state.uplink.length ? parseFloat(this.state.uplink[0].power_atten) : 0, label: this.state.uplink.length ? this.state.uplink[0].power_atten : 0},
        {  x: this.state.uplink.length ? this.state.uplink[1].timestamp  : 0, y: this.state.uplink.length ? parseFloat(this.state.uplink[1].power_atten) : 0, label: this.state.uplink.length ? this.state.uplink[1].power_atten : 0},
        {  x: this.state.uplink.length ? this.state.uplink[2].timestamp  : 0, y: this.state.uplink.length ? parseFloat(this.state.uplink[2].power_atten) : 0, label: this.state.uplink.length ? this.state.uplink[2].power_atten : 0},
        {  x: this.state.uplink.length ? this.state.uplink[3].timestamp  : 0, y: this.state.uplink.length ? parseFloat(this.state.uplink[3].power_atten) : 0, label: this.state.uplink.length ? this.state.uplink[3].power_atten : 0},
        {  x: this.state.uplink.length ? this.state.uplink[4].timestamp  : 0, y: this.state.uplink.length ? parseFloat(this.state.uplink[4].power_atten) : 0, label: this.state.uplink.length ? this.state.uplink[4].power_atten : 0},
        {  x: this.state.uplink.length ? this.state.uplink[5].timestamp  : 0, y: this.state.uplink.length ? parseFloat(this.state.uplink[5].power_atten) : 0, label: this.state.uplink.length ? this.state.uplink[5].power_atten : 0},
        {  x: this.state.uplink.length ? this.state.uplink[6].timestamp  : 0, y: this.state.uplink.length ? parseFloat(this.state.uplink[6].power_atten) : 0, label: this.state.uplink.length ? this.state.uplink[6].power_atten : 0},
         ]
    }
    if (this.state.uplinkChart === 'yesterday') {
         return [
      {  x: Unix_timestamp(this.state.uplink.length ? this.state.uplink[this.state.yma].timestamp ? this.state.uplink[this.state.yma].timestamp : 0 : 0), y: this.state.uplink.length ? parseFloat(this.state.uplink[this.state.yma].power_atten) ? parseFloat(this.state.uplink[this.state.yma].power_atten) : 0 : 0, label: this.state.uplink.length ? this.state.uplink[this.state.yma].power_atten ? this.state.uplink[this.state.yma].power_atten : 0 : 0},
      {  x: Unix_timestamp(this.state.uplink.length ? this.state.uplink[this.state.ymb].timestamp ? this.state.uplink[this.state.ymb].timestamp : 0 : 0), y: this.state.uplink.length ? parseFloat(this.state.uplink[this.state.ymb].power_atten) ? parseFloat(this.state.uplink[this.state.ymb].power_atten) : 0 : 0, label: this.state.uplink.length ? this.state.uplink[this.state.ymb].power_atten ? this.state.uplink[this.state.ymb].power_atten : 0 : 0},
      {  x: Unix_timestamp(this.state.uplink.length ? this.state.uplink[this.state.ymc].timestamp ? this.state.uplink[this.state.ymc].timestamp : 0 : 0), y: this.state.uplink.length ? parseFloat(this.state.uplink[this.state.ymc].power_atten) ? parseFloat(this.state.uplink[this.state.ymc].power_atten) : 0 : 0, label: this.state.uplink.length ? this.state.uplink[this.state.ymc].power_atten ? this.state.uplink[this.state.ymc].power_atten : 0 : 0},
      {  x: Unix_timestamp(this.state.uplink.length ? this.state.uplink[this.state.ymd].timestamp ? this.state.uplink[this.state.ymd].timestamp : 0 : 0), y: this.state.uplink.length ? parseFloat(this.state.uplink[this.state.ymd].power_atten) ? parseFloat(this.state.uplink[this.state.ymd].power_atten) : 0 : 0, label: this.state.uplink.length ? this.state.uplink[this.state.ymd].power_atten ? this.state.uplink[this.state.ymd].power_atten : 0 : 0},
      {  x: Unix_timestamp(this.state.uplink.length ? this.state.uplink[this.state.yme].timestamp ? this.state.uplink[this.state.yme].timestamp : 0 : 0), y: this.state.uplink.length ? parseFloat(this.state.uplink[this.state.yme].power_atten) ? parseFloat(this.state.uplink[this.state.yme].power_atten) : 0 : 0, label: this.state.uplink.length ? this.state.uplink[this.state.yme].power_atten ? this.state.uplink[this.state.yme].power_atten : 0 : 0},
      {  x: Unix_timestamp(this.state.uplink.length ? this.state.uplink[this.state.ymf].timestamp ? this.state.uplink[this.state.ymf].timestamp : 0 : 0), y: this.state.uplink.length ? parseFloat(this.state.uplink[this.state.ymf].power_atten) ? parseFloat(this.state.uplink[this.state.ymf].power_atten) : 0 : 0, label: this.state.uplink.length ? this.state.uplink[this.state.ymf].power_atten ? this.state.uplink[this.state.ymf].power_atten : 0 : 0},
      {  x: Unix_timestamp(this.state.uplink.length ? this.state.uplink[this.state.ymg].timestamp ? this.state.uplink[this.state.ymg].timestamp : 0 : 0), y: this.state.uplink.length ? parseFloat(this.state.uplink[this.state.ymg].power_atten) ? parseFloat(this.state.uplink[this.state.ymg].power_atten) : 0 : 0, label: this.state.uplink.length ? this.state.uplink[this.state.ymg].power_atten ? this.state.uplink[this.state.ymg].power_atten : 0 : 0}
      ]
    }
     else { const arraytod = [];
          if (this.state.uplink.length > 13) {
          for(let i = this.state.uplink.length - 14;i<this.state.uplink.length;i++) {
              if (this.state.uplink[i].scrap_id == this.state.id) {
                  arraytod.push({  x: Unix_timestamp(this.state.uplink.length ? this.state.uplink[i].timestamp ? this.state.uplink[i].timestamp : 0 : 0), y: this.state.uplink.length ? parseFloat(this.state.uplink[i].power_atten) ? parseFloat(this.state.uplink[i].power_atten) : 0 : 0, label: this.state.uplink.length ? this.state.uplink[i].power_atten ? this.state.uplink[i].power_atten : 0 : 0})
                }
            }
          }
              if (this.state.uplink.length < 14 && this.state.uplink.length > 0) {
    ...
    return (
        ...
        <ScrollView contentContainerStyle={{ paddingVertical: 25 }}>
          <View style = {styles.inputBox}>
                ...
                >
                    <Picker.Item label="Puskesmas Lembah Binuang" value="m2" />
          ...
          <Card
            ...
            <Block>
              <VictoryChart width={350} theme={VictoryTheme.material} domainPadding={15}>
                <VictoryLine
